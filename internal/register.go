...
func Register(ctx *godog.ScenarioContext) {
	...
	ctx.Step("^eventually value (.*) equals (.*) with (.*)$", eventuallyValueAssertEqualsWithOptions)
	ctx.Step("^eventually value (.*) equals (.*)$", eventuallyValueAssertEqualsNoOptions)
	ctx.Step("^value (.*) is zero$", valueAssertIsZero)
	ctx.Step("^eventually value (.*) is zero with (.*)$", eventuallyValueAssertIsZeroWithOptions)
	ctx.Step("^eventually value (.*) is zero$", eventuallyValueAssertIsZeroNoOptions)
	ctx.Step("^value (.*) is not zero$", valueAssertIsNotZero)
	ctx.Step("^eventually value (.*) is not zero with (.*)$", eventuallyValueAssertIsNotZeroWithOptions)
	ctx.Step("^eventually value (.*) is not zero$", eventuallyValueAssertIsNotZeroNoOptions)
